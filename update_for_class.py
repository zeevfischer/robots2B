...
def process_img(image_path, output_csv, output_image):
    ...
    if ids is not None:
        for i in range(len(ids)):
            ...
            output_data.append([0,ids[i][0], corner_points_list, distance, yaw, pitch, roll])

        # Save the image with detected QR codes
        cv2.imwrite(output_image, image)

    # Write the output data to CSV
    columns = ['Frame ID','QR ID', 'QR 2D (Corner Points)', 'Distance', 'Yaw (degrees)', 'Pitch (degrees)', 'Roll (degrees)']
...
def parse_corners(corner_str):
    try:
        ...
        clean_str = re.sub(r'array\(|\)', '', corner_str).replace('\n', '')
        return np.array(ast.literal_eval(clean_str))
    except Exception as e:
        print(f"Error parsing corners: {e}")
        return corner_str
# Function to calculate yaw, pitch, distance, and 2D corner point movement commands
def calculate_movement_commands(reference_info, current_info):
    commands = []

    # Convert reference_info corners from string to numpy array
    for ref in reference_info:
        ref[2] = parse_corners(ref[2])
    ...
    for i in range(len(current_info)):
        for j in range(len(reference_info)):
            ...
            what_we_reference = int(reference_info[j][1])
            if what_i_see == what_we_reference:
                ref_distance = reference_info[j][3]
                ref_yaw = reference_info[j][4]
                ref_pitch = reference_info[j][5]
                ref_corners = np.array(reference_info[j][2])
                ...
                cur_corners = np.array(current_info[i][5])

                # Ensure ref_corners and cur_corners are not empty and have the correct shape
                if ref_corners.size == 0 or cur_corners.size == 0:
                    continue

                if ref_corners.shape != (4, 2) or cur_corners.shape != (4, 2):
                    continue
                ...
                corner_diff_x = np.mean(ref_corners[:, 0] - cur_corners[:, 0])
                corner_diff_y = np.mean(ref_corners[:, 1] - cur_corners[:, 1])
                ...
                print(f"Differences    - Aruco: {current_info[i][0]:.2f} Distance: {distance_diff:.2f}, Yaw: {yaw_diff:.2f}, Pitch: {pitch_diff:.2f}, Corner Diff X: {corner_diff_x:.2f}, Corner Diff Y: {corner_diff_y:.2f}")

                yaw_command = None
                distance_command = None
                pitch_command = None
                corner_command_x = None
                corner_command_y = None
                ...
                if abs(corner_diff_x) > 15:  # Arbitrary threshold for corner X difference
                    if corner_diff_x > 0:
                        corner_command_x = "left"
                    else:
                        corner_command_x = "right"

                if abs(corner_diff_y) > 15:  # Arbitrary threshold for corner Y difference
                    if corner_diff_y > 0:
                        corner_command_y = "up"
                    else:
                        corner_command_y = "down"
                command = [current_info[i][0], yaw_command, distance_command, pitch_command, corner_command_x,corner_command_y]
...
def draw_directions(frame, commands):
    ...
    for i in range(len(commands)):
        ...
        if commands[i][4]:
            direction_text += f"Corner X: {commands[i][4]}  "
        if commands[i][5]:
            direction_text += f"Corner Y: {commands[i][5]}  "
...
def process_live_frame(frame, frame_id, csv_file, output_frames_dir, reference_info):
    ...
    if ids is not None:
        ...
        for i in range(len(ids)):
            ...
            current_info.append([ids[i][0],distance, yaw, pitch, roll,corner_points_list])
...
def display_live_stream(csv_file, output_frames_dir, output_csv_video):
    ...
    try:
        while True:
            ...
            if end_of_csv and reference_info == []:
                print("end of file")
                break
...
reference_image_path = 'data\\2B\\img1.jpg'
...
# img_path = 'data\\imgm1.jpg'

# Set up directories
# setup_directories(output_directories)

# process_video(video_path, output_csv_video, output_video, output_frames_dir)
process_img(reference_image_path,output_csv,'output\\img.jpg')
display_live_stream(output_csv_live, 'output\\frames',output_csv)